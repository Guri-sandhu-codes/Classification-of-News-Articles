...
# MongoDB connection
mongo_uri = "mongodb+srv://Gurider:Gsm#2000@atlascluster.4qfhmkj.mongodb.net/?retryWrites=true&w=majority&appName=AtlasCluster"  # Replace with your actual MongoDB URI
...
def predict():
    if request.method == 'POST':
        ...
        data = {
            'news': news,
            'predicted_category': result.get(int(prediction)),
            'top_prediction': {
                'category': result.get(top_two_indices[1]).capitalize(),
                'accuracy': round(predicted_category[0][top_two_indices[1]] * 100, 2)
            },
            'second_prediction': {
                'category': result.get(top_two_indices[0]).capitalize(),
                'accuracy': round(predicted_category[0][top_two_indices[0]] * 100, 2)
            }
        }

        # Insert the prediction result into MongoDB
        collection.insert_one(data)
        #round(predicted_category[0][top_two_indices[0]] * 100,
                                                                     #2)))
        return render_template('resultt.html',news = news, prediction=result.get(int(prediction)), topPrediction = result.get(top_two_indices[1]).capitalize(), topAcc= round(predicted_category[0][top_two_indices[1]] * 100, 2),
